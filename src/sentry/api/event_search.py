...
key_val_term         = space? (tag_filter / time_filter / rel_time_filter / specific_time_filter
                       / numeric_filter / result_filter / result_date_filter / has_filter / is_filter / basic_filter)
                       space?
...
result_filter        = aggregate_key sep operator? numeric_value
result_date_filter   = aggregate_key sep operator? (date_format / rel_date_format)
...
class SearchVisitor(NodeVisitor):
    ...
    def visit_result_date_filter(self, node, children):
        (search_key, _, operator, search_value) = children
        ...
        if is_date_aggregate:
            ...
            return ResultFilter(search_key, operator, SearchValue(search_value))
        else:
            search_value = operator + search_value if operator != "=" else search_value
            return ResultFilter(search_key, "=", SearchValue(search_value))
